...
void Issue()
{
    ...
    while(newnode!=NULL)    
    {
             if(newnode->source_1_ready==1 && newnode->source_2_ready==1 && execute_list_counter<fetch_size)
             {   newnode->status=1;
...
{   
    ...
    while(newnode!=NULL)    
    {
             ...
             {   
                 ROB[newnode->tag].IF_time=ROB[newnode->tag].arrival_clock;
                 ROB[newnode->tag].current_state=1;
                 newnode->current_state++;
             } 
             else if(newnode->current_state==1)
             {  
                 ROB[newnode->tag].ID_time=ROB[newnode->tag].arrival_clock+1;
